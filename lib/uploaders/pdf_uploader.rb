class PdfUploader < CarrierWave::Uploader::Base
  ...
  version :s_160 do
    ...
    process :resize_and_pad => [160, nil, 'white', 'Center']
    def full_filename(for_file = model.file_name.file)
      "s_160_#{for_file.sub('.pdf', '.avif')}"
    end
    def style = :s_160
  ...
  version :m_400 do
    ...
    process :resize_and_pad => [400, nil, 'white', 'Center']
    def full_filename(for_file = model.file_name.file)
      "m_400_#{for_file.sub('.pdf', '.avif')}"
    end
    def style = :m_400
  ...
  version :l_1000 do
    ...
    process :resize_and_pad => [1000, nil, 'white', 'Center']
    def full_filename(for_file = model.file_name.file)
      "l_1000_#{for_file.sub('.pdf', '.avif')}"
    end
    def style = :l_1000
