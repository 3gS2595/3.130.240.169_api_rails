...
class TumblrApiInit 
  ...
  def perform()
      ...
      @subsets.filter { |s| s.time_last_scraped_completely == nil }.each do |s|
        if src_user == nil && !Event.where(info: s.url).exists?(['updated_at >= ?', (DateTime.now - ((10/60)/24))])
          ...
          # if incompleted initialization, updates event if stale
            ...
            if (Time.now - @this_event.event_time.to_time) > @this_event.duration_limit
              ...
              src_user = s
              puts 'INIT STALE :: ' + (Time.now - @this_event.event_time.to_time).to_s + ' > ' + @this_event.duration_limit.to_s
              puts 'RESUMING INITIALIZATION: ' + s.url.split('/').last + ' starting at: ' + @this_event.busy_objects
            end
          end
        end
      end
