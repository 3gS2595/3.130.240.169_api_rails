...
class TumblrApi  
  ...
  def perform()
    ...
    client = Tumblr::Client.new({
      ...
      cnt_requests = 0
      @subsets = SrcUrlSubset.all.reverse
      @todo = []
      ...
      if @todo.length == 0 
        ...
        @todo.each do | new_s |
          if !Event.exists?(info: new_s.url)
            puts 'new unfinished event'
            @event = Event.create(
              status: 'scrape inprogress',
              event_time: DateTime.now(),
              info: new_s.url,
              permissions: new_s.permissions,
              origin: 'tumblr_api.rb',
              duration_limit: 900
            )
          else
            puts'resuming ' + new_s.url
          end
        end
      end
        ...
        if src_user.url.include? ('tumblr')
          ...
          if Event.exists?(info: src_user.url)
            ...
            while cnt_post_offset <= cnt_total_posts || cnt_total_posts == -1
              ...
              if Event.exists?(info: src_user.url)
                Event.where(info: src_user.url)[0].update(event_time: DateTime.now(), duration_limit: cnt_post_offset, status: 'in progress')
              end
            end
          end
        end
